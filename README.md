...
- 调整显示与刷新偏好,即时生效并写入 `Data\settings.json`:
  ...
  - **每个源的圆环颜色**(留空 = 按用量着色)
  - **网络代理**(跟随系统/直连/手动 HTTP;与液态玻璃一样**重启后生效**)

### 关于"液态玻璃"与背景透明度

默认**关闭**液态玻璃,面板用 80% 不透明黑 + 桌面混合,圆弧与半透明都正常。
若打开液态玻璃(`SetWindowCompositionAttribute` 的 acrylic 模糊),要注意两条实测结论:

- 模糊区域只能是**整个窗口矩形**,跟不了 rail/卡片的圆弧轮廓 —— 开着时圆角会变直角,
  所以默认关(保留开关仅供想看模糊效果时试用)。
- 该 API 一旦调用,分层窗口的 **per-pixel alpha 就失效**,且**无法在当前进程内恢复**。
  因此玻璃开关按**重启生效**处理(与代理同理):运行中切换不会立刻改观,且关闭后要
  重启程序圆弧才会回来。**不确定就别开。**

> v1.7.2 修复的正是这里:此前"关玻璃"的分支也会调一次 `ACCENT_DISABLED`,导致每次
> 启动都把 alpha 打坏 —— rail 与卡片成了一整块不透明黑矩形,圆角看不见、桌面透不出来。
> 现在**没开过玻璃就完全不碰**这个 API。
