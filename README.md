...
**DSH 的子代理会话会归根到主会话**。DSH 每派一个子代理就起一个独立会话(自己的会话文件、
自己的标题,如"任务: 把 `D:\…\05CADTools`…"),父子关系记在**父会话**的
`subagent/catalog` 事件里(`childId`)。PulseWin 沿这条线索把子会话的用量、项目、标题
都换成它所属的主会话的——否则"最近会话"会被拆成一堆看起来互不相干的任务,而用户认的是
"我开的那个任务"。找不到父(父会话文件已被删)的**原样保留**,不做猜测。
去重键用的是**来源自己的会话号**(不是归根后的分组号),所以归根规则怎么改都不会让
历史明细被重复计数——改这条逻辑时务必保持这个区分。
