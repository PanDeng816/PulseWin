...
- **工具窗按需读取、用完即还**:模型页与用量统计窗共用**同一份账本**(引用计数,
...
设置 → **模型**(左侧"数据"分组下),把**本机用过的每个 AI 模型**列成一行,点一行看它的详情:

- **列表**:模型名 + 相对最大用量的一条进度条 + 总量 / 调用次数 / 缓存命中率 + 估算金额
- **详情**:KPI 四宫格(总 token、估算金额、调用次数、会话数)、Token 四类构成、
  **逐日用量柱状图**、**用量集中在哪些项目**、以及该模型的公开牌价(每百万 token)
- 区间与统计窗口一样(今天 / 7 天 / 30 天 / 全部)

数据来源与统计窗口同源(本机 ZCode / OpenCode 记录库,不发请求),口径也完全一致:
同一个模型的两种写法(`deepseek/deepseek-v4.1-flash` 与 `deepseek-v4.1-flash`)归成一行,
无公开价的模型显示 `—` 而不是 `¥0.00`。

> **为什么不用 Command Code 的逐模型接口**:它的 API Key 只够读账户级数字;逐模型的
> 明细在 `/internal/usage/*` 下,**只对浏览器登录态开放**(本机实测:API Key 加
> Origin/Referer 伪装也是 401)。所以本页的每模型数据一律来自本机记录库——可靠、
> 不发请求、不依赖登录。

### 可选:浏览器会话明细(默认关)

模型页有一个**浏览器会话明细**开关,开了之后会读本机浏览器(Edge/Chrome)的登录 cookie,
...
**不开这个开关,模型页也完整可用**(本地库本来就有逐模型的 token 与缓存读/写)。
...
| `--models` | 把逐模型聚合与浏览器会话通道的状态打出来 | `Data\models-dump.txt` |
